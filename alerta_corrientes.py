...
import main  # Importamos tu bot para disparar las alertas

def obtener_datos_open_meteo(lat, lon):
    """Consulta la API de Open-Meteo para unas coordenadas específicas."""
    # Pedimos la precipitación actual y el código de estado del clima (WMO)
...
def decodificar_alerta_wmo(weather_code, precipitacion):
    """
    Convierte los códigos WMO de Open-Meteo a los niveles de alerta del SAPH.
    """
    # Tormentas eléctricas (95) y con granizo (96, 99)
    if weather_code in [95, 96, 99]:
        return 4, "ALERTA ROJA: Tormenta severa o granizo detectado."
    
    # Lluvia fuerte (65) o chubascos violentos (82)
    elif weather_code in [65, 82] or precipitacion > 10.0:
        return 3, "ALERTA NARANJA: Lluvia muy fuerte, posible anegamiento."
    
    # Lluvia moderada (61, 63) o chubascos (80, 81)
    ...
    # Despejado, nublado, niebla (0, 1, 2, 3, 45, 48)
    else:
...
def demo_alerta_corrientes():
    ...
    # 1. Definimos los barrios y sus coordenadas GPS reales
    barrios_gps = {
    ...
    for nombre_barrio, coords in barrios_gps.items():
        ...
        if datos and "current" in datos:
            ...
            # Si el nivel es de precaución (Amarillo o superior), lo agregamos a la lista de afectados
            if nivel >= 2:
...
# 3. Guardar el registro general en JSON
    datos_json = {
...
# 4. Integración con el Bot (SAPH)
    mapa_niveles = {0: "Despejado", 1: "Verde", 2: "Amarilla", 3: "Naranja", 4: "Roja"}
    
    # 🔧 MODO PRUEBA: Si todo está despejado, forzamos un simulacro para probar el bot
